...
class baseconfig(connection_methods):
    def __str__(self):
        return ("<Name: {name}, ID: {id}, Active: {active}>".format(name=self.name.encode('utf-8'), id=self.id.encode('utf-8'), active=self.active.encode('utf-8')))

    def __repr__(self):
        return "{0} - {1} - {2}.".format(self.name.encode('utf-8'),self.id.encode('utf-8'),self.active.encode('utf-8'))
    ...
    def clone(self, newname, newplaceid):
        clone_url = "duplicateobject.htm?id={objid}&name={name}&targetid={newparent}".format(objid=self.id,
                                                                                             name=newname,
                                                                                             newparent=newplaceid)
