...
def sync_puppet_config(hostgroup,hostvars,user="NULL"):
    ...
    cmd = "ansible-playbook site.yml -e 'project={0}{1} user={2} pupm=puppmaster-b2c' -f10 --skip-tags 'pupagent'".format(hostgroup,hvaransi,user)
    print cmd

    res = executor.run_command(cmd, changedir=PUPPET_DIR, bufferResult=True, lock=[hostgroup, user])
    return True, res

@app.task(name="argus_puppet_worker.test_rei")
def sync_puppet_config_rei(hostgroup,hostvars,user="NULL"):
    get_puppet_sync_config(hostgroup,newkey=hostvars)
