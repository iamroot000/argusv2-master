...
class dockerSession(object):
    ...
    def __init__(self, url, user=None, password=None):
        self.__conn = Client(base_url=url,timeout=5)
        self.__CREDENTIALS = {"username": user, "password": password}

    def removeContainer(self, containerName):
        try:
            # hope that cadvisor doesnt fuck up and get the container stuck
            self.__conn.stop(containerName)
    ...
    def downloadLatestImage(self, reponimage):
        ...
        try:
            self.__conn.pull(reponimage, LATEST_TAG,
                             auth_config=self.__CREDENTIALS)
        except Exception as exception:
    ...
    def backupCurrentContainerImage(self, containerName, reponimage):
        ...
        '''
        print("Creating backup image for %s to %s.." %
              (containerName, reponimage))
        try:
            ...
            self.__conn.push(reponimage, backuptag,
                             auth_config=self.__CREDENTIALS)
        except Exception as exception:
    ...
    def pushContainerToLatest(self, containerName, reponimage=None):
        ...
        try:
            ...
            self.__conn.push(reponimage, LATEST_TAG,
                             auth_config=self.__CREDENTIALS)
        except Exception as exception:
            print("Failure..\n%s" % str(exception))
        return True
    ...
    def getContainerSpecifications(self, container_name):
        ...
        rVal['id'] = profile['Id']
        #rVal['ports'] = hostconfig['PortBindings'].keys() if hostconfig['PortBindings'] else [];
        rVal['ports'] = [k + "->" + "/".join([i['HostPort'] for i in v])
                         for k, v in hostconfig['PortBindings'].iteritems()] if hostconfig['PortBindings'] else []
        rVal['networks'] = {k: {'ip': v['IPAddress'], 'mac': v['MacAddress'],
                                'gateway': v['Gateway']} for k, v in network['Networks'].iteritems()}
