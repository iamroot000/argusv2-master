...
def pageDisplay(request):
    ...
    for exp in query_set:
        ...
        try:
            ...
            sslDomain2.objects.filter(domain=exp.domain).update(daysleft=daysleft,status=status,expiration=dom_expiry)
        except:
            # sslDomain.objects.filter(domain=exp.domain).update(status=2)
            sslDomain2.objects.filter(domain=exp.domain).update(status=2)
