...
class SSRinitForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        condigdata = json.loads(config_read)
        IDC_CHOICES = [('---','---')]
        V2RAY_DOMAIN_CHOICES = [('---', '---')]
        for i in condigdata["configlist"]["IDC"]:
            IDC_CHOICES.append((i, i))
        self.fields['IDC'].choices = IDC_CHOICES


        for i in condigdata["configlist"]["V2RAY_DOMAIN"]:
            V2RAY_DOMAIN_CHOICES.append((i, i))
        self.fields['V2RAY_DOMAIN'].choices = V2RAY_DOMAIN_CHOICES

    IP = forms.CharField(max_length=960, widget=forms.Textarea(attrs={'rows':5, 'cols': 16, 'placeholder': "Enter IP Address"}), label="IP address")
    ...
    V2RAY_PORT = forms.CharField(max_length=5, initial=23456)
    V2RAY_PATH = forms.CharField(max_length=50, initial='awesomepa')
    V2RAY_DOMAIN = forms.ChoiceField(
        label="V2ray domain", initial='---'
    )
    ...
    def clean_V2RAY_DOMAIN(self):
        V2RAY_DOMAIN = self.cleaned_data.get("V2RAY_DOMAIN")
        if V2RAY_DOMAIN == '---':
            raise forms.ValidationError("Incorrect V2ray Domain!")
        else:
            return V2RAY_DOMAIN



    def clean_V2RAY_PORT(self):
        V2RAY_PORT = self.cleaned_data.get("V2RAY_PORT")
        try:
            if int(V2RAY_PORT) <= 65535 and int(V2RAY_PORT) > 0:
                return V2RAY_PORT
            else:
                raise forms.ValidationError("Port is out of range.")
        except ValueError:
...
class V2rayconfigForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super(V2rayconfigForm, self).__init__(*args, **kwargs)
        config_file = os.path.join(settings.BASE_DIR, 'SSRCHECKER/aom/ansible/config.json')
        config_read = open(config_file, 'r').read()
        condigdata = json.loads(config_read)
        V2RAY_DOMAIN_CHOICES = [('---', '---')]
    ...
    IP = forms.CharField(max_length=960, widget=forms.Textarea(attrs={'rows':5, 'cols': 16, 'placeholder': "Enter V2ray IP"}), label="IP address")
    V2RAY_PORT = forms.CharField(max_length=5, initial=23456)
    V2RAY_PATH = forms.CharField(max_length=50, initial='awesomepa')
    V2RAY_DOMAIN = forms.ChoiceField(
        label="V2ray domain", initial='---'
