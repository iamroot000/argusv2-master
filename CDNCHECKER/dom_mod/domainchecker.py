import requests, logging, datetime, json, sqlite3, time, pytz, mysql.connector, os, sys
...
class DomainSystem():
    def __init__(self,conn_sql=False):
        BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        sqldir = os.path.join(BASE_DIR, 'CDNdomain.sqlite3')
        ...
        if self.conn_sql is True or self.conn_sql == "True":
            ...
            self.db_conn = mysql.connector.connect(host=self.mydb_host, database=self.mydb_db, user=self.mydb_user,
                                    passwd=self.mydb_pass)
        else:
    ...
    def loggingFile(self,log_debug=None,log_info=None,log_warning=None,log_error=None,log_critical=None):
        BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        logdir = os.path.join(BASE_DIR, 'dom_mod/logs/')
    ...
    def domain_store(self):
        try:
            mysql_q = MYSQLquery()
            self.db_conn.execute("CREATE TABLE {}({} INTEGER PRIMARY KEY, {} TEXT, {} TEXT, {} TEXT, {} TEXT, {} TEXT)".format(self.db_table, self.db_columid, self.db_columnp, self.db_column1, self.db_column2, self.db_column3, self.db_column4))
...
#                d_status = "TESTING"
                if self.conn_sql is True or self.conn_sql == "True":
                    ...
                    self.cursor.execute("UPDATE {0} SET {1}=%s, {2}=%s, {3}=%s WHERE {4}=%s ".format(self.db_table, self.db_column2, self.db_column3, self.db_column4, self.db_column1), params)
                    self.loggingFile(log_info="{} has been updated to the MYSQL database - PARAMS={}".format(d_list,params))
                else:
                    params = (
                    str(d_status), str(datetime.datetime.now(pytz.timezone('Asia/Manila'))).split('+')[0], "---",
                    str(d_list[0]))
                    ...
                    self.loggingFile(log_info="{} has been updated to the SQLITE database - PARAMS={}".format(d_list,params))
                self.db_conn.commit()
            self.db_conn.close()
...
if __name__ == "__main__":
    DomainSystem(conn_sql=True).domain_store()
#    print(DomainSystem(conn_sql=True).domain_status("kbonlive.cn"))
